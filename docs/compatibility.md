...
Current baseline: **1294 accepted / 1809 total; 515 unresolved**.
...
| ENT-PICKUP | Sun, coins, prizes, and pickup behavior | 13 | 26 | partial | SunPickupState, CoinPickupState, SunProduced/SunCollected/CoinProduced/CoinCollected events, CollectSun/CollectCoin input actions, and source-like COIN_MOTION_COIN award arcs (launch, drift, gravity, item-award offsets, sunflower award elevation, landing) are implemented; the source pickup catalog, money/sun variants, mode-unlock presents, garden/usable-seed prizes, chocolate, and award-bag currency effects are covered by ignored `artifacts/pickups/verification.md`, `artifacts/pickup-arcs/verification.md`, and core tests; visual pickup particles and remaining award-specific progression unresolved |
...
| AUD-SFX | Simulation-tick and decoded-output sound-event units | 34 | 167 | partial | `SeedSelected` → `sounds/tap.ogg`, `InputRejected` → `sounds/buzzer.ogg`, `Paused` → `sounds/pause.ogg`, `PlantPlaced` → `sounds/plant.ogg`, `PlantShoveled` → `sounds/plant2.ogg`, `SunCollected` → `sounds/points.ogg`, `CoinCollected` → `sounds/coin.ogg`, `GardenWatered` → `sounds/watering.ogg`, `GardenFertilized` → `sounds/fertilizer.ogg`, IceShroom `PlantSpecialTriggered` → `sounds/frozen.ogg`, `ZombieChilled` → `sounds/frozen.ogg`, `CobCannonFired` → `sounds/coblaunch.ogg`, `PortalOpened` → `sounds/portal.ogg`, GraveBuster `PlantSpecialTriggered` → `sounds/gravebusterchomp.ogg`, Coffee `PlantSpecialTriggered` → `sounds/coffee.ogg`, TangleKelp `TangleKelpGrabStarted` → `sounds/floop.ogg`, TangleKelp `TangleKelpWaterEntry` → `sounds/zombiesplash.ogg`, PotatoMine `PotatoMineArmed` → `sounds/dirt_rise.ogg`, Digger `DiggerSurfaced` → `sounds/dirt_rise.ogg`, Magnet-shroom `MetalStolen` → `sounds/magnetshroom.ogg`, Zamboni `VehicleDisabled` → `sounds/balloon_pop.ogg`, PotatoMine `PlantSpecialTriggered` → `sounds/potato_mine.ogg`, Spikeweed `PlantSpecialTriggered` → `sounds/throw.ogg`, CherryBomb `PlantSpecialTriggered` → `sounds/cherrybomb.ogg`, Jalapeno `PlantSpecialTriggered` → `sounds/jalapeno.ogg`, CherryBomb/Jalapeno companion → `sounds/juicy.ogg`, DoomShroom `PlantSpecialTriggered` → `sounds/doomshroom.ogg`, `BloverTriggered` → `sounds/blover.ogg`, Chomper `PlantSpecialTriggered` → `sounds/bigchomp.ogg`, `ZombieHypnotized` → `sounds/mindcontrolled.ogg`, `JackboxExploded` → `sounds/explosion.ogg`, `MowerTriggered` → `sounds/lawnmower.ogg`, `GameLost` → `sounds/losemusic.ogg`, `GameWon` → `sounds/winmusic.ogg`; source cross-checks, decoded PCM hashes, and local app tick/`playback started` traces in ignored `artifacts/local-audio/verification.md`; `Resumed` deliberately has no sound mapping; `ZombieDied` splat variation and remaining SFX/device timing remain under Issue `#19` |
