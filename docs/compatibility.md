...
Current baseline: **1239 accepted / 1809 total; 570 unresolved**.
...
| ENT-EFFECT | Player-observable particle/effect events | 58 | 105 | partial | Ignored `artifacts/effect-evidence/catalog.md` (full 106-entry trigger-site catalog) and `artifacts/effect-anchors/verification.md`; 54 simulation-class effects have deterministic anchor events and focused tests (splats, specials, planting, grave/vase lifecycle, armor/shield drops, thaw, potato arm, tallnut jump block, pogo break, digger rise, mind control, vehicle deaths, ice-trail state, portal open/teleport, umbrella deflect, butter); remaining units ride the boss system and the renderer campaign; Issues `#2`, `#5` |
