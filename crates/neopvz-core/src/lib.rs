...
const SNORKEL_EAT_TRANSITION_TICKS: u32 = 100;
// HEIGHT_OUT_OF_POOL rises from -90 at two pixels per tick for a Snorkel.
const SNORKEL_EXIT_TICKS: u32 = 45;
const SNORKEL_WALKING_PHASE: u8 = 0;
const SNORKEL_INTO_POOL_PHASE: u8 = 1;
const SNORKEL_WALKING_IN_POOL_PHASE: u8 = 2;
const SNORKEL_UP_TO_EAT_PHASE: u8 = 3;
const SNORKEL_EATING_IN_POOL_PHASE: u8 = 4;
const SNORKEL_DOWN_FROM_EAT_PHASE: u8 = 5;
const SNORKEL_OUT_OF_POOL_PHASE: u8 = 6;
...
impl PlantType {
    ...
    // Plant::FindTargetAndFire and UpdateShooting (Plant.cpp:714-800, 3235-3339).
    fn shooting_counter_start(self) -> u32 {
        match self.slot() {
            7 | 28 | 52 => 26,
            40 => 100,
            43 | 10 => 50,
    ...
    fn fires_at_shooting_counter(self, counter: u32) -> bool {
        match self.slot() {
            40 => matches!(counter, 68 | 51 | 35 | 18),
            43 => counter == 19,
            42 => matches!(counter, 126 | 98 | 70 | 42),
            _ => counter == 1,
        }
    }
...
impl Game {
        ...
        for index in 0..plant_count {
            ...
            let mut fired_directions = 0;
            {
                ...
                if plant.shooting_counter > 0 {
                    ...
                    if fire {
                        fired_directions = std::mem::take(&mut plant.firing_directions);
                        firing_projectile = plant.kernel_pult_projectile.take();
                    }
                }

                let mut attack_directions = None;
                if plant.launch_rate != 0 {
                    if plant.launch_counter <= 1 {
                        if plant_type.is_producer() {
                            ...
                            attack_directions =
                                Some(if plant_type.firing_pattern() == FiringPattern::Split {
                                    firing_directions & FIRING_BACKWARD
                                } else {
                                    firing_directions
                                });
                        }
                        ...
                        // Plant::UpdateShooter checks these boundaries after decrementing.
                        if (plant_type.firing_pattern() == FiringPattern::Homing
                            && plant.launch_counter == 50)
                            || (matches!(plant_type.slot(), 7 | 28 | 52)
                                && plant.launch_counter == 25)
                        {
                            attack_directions = Some(firing_directions);
                        }
                    }

                    if let Some(directions) = attack_directions {
                        let attack_has_target = match plant_type.firing_pattern() {
                            FiringPattern::Split | FiringPattern::Backward => directions != 0,
                            _ => has_target,
                        };
                        if attack_has_target {
                            plant.firing_directions = directions;
                            ...
                            plant.shooting_counter = plant_type.shooting_counter_start();
                        }
                    }
                }
            }
    ...
    fn update_snorkel_state(&mut self, zombie_index: usize, events: &mut Vec<GameEvent>) {
        let (entity, row, backwards) = {
            let zombie = &self.state.board.zombies[zombie_index];
            if zombie.zombie_type != ZombieType::Snorkel {
                return;
            }
            (zombie.id, zombie.row, zombie.hypnotized)
        };
        if self.state.scene != SceneKind::Pool || !matches!(row, 2 | 3) {
            return;
        }
        ...
        let mut pool_splash = false;
        {
            ...
            match zombie.snorkel_phase {
                ...
                SNORKEL_WALKING_IN_POOL_PHASE => {
                    if (!backwards && zombie.position_x <= 25 * POSITION_SCALE)
                        || (backwards && zombie.position_x > 640 * POSITION_SCALE)
                    {
                        zombie.snorkel_phase = SNORKEL_OUT_OF_POOL_PHASE;
                        zombie.snorkel_counter = SNORKEL_EXIT_TICKS;
                        zombie.position_x += if backwards {
                            15 * POSITION_SCALE
                        } else {
                            -15 * POSITION_SCALE
                        };
                        ...
                        zombie.snorkel_counter = SNORKEL_EAT_TRANSITION_TICKS;
                        zombie.speed = 0;
                    }
                }
                SNORKEL_UP_TO_EAT_PHASE => {
                    zombie.speed = 0;
                    if !zombie.eating {
                        zombie.snorkel_phase = SNORKEL_DOWN_FROM_EAT_PHASE;
                        zombie.snorkel_counter = SNORKEL_EAT_TRANSITION_TICKS;
                    } else {
                        zombie.snorkel_counter = zombie.snorkel_counter.saturating_sub(1);
                        if zombie.snorkel_counter == 0 {
                            zombie.snorkel_phase = SNORKEL_EATING_IN_POOL_PHASE;
                        }
                    }
                }
...
fn snorkel_blocks_attack(zombie: &ZombieState) -> bool {
    zombie.zombie_type == ZombieType::Snorkel
        && matches!(
            zombie.snorkel_phase,
            SNORKEL_INTO_POOL_PHASE
...
mod tests {
    ...
    fn splitpea_and_leftpeater_arm_only_matching_directions() {
        let run = |plant_type: PlantType, launch_counter, targets: &[(u8, i64)]| {
            let mut game = Game::new(7, SceneKind::Day);
            game.state
                .board
                .plants
                .push(test_plant(100, plant_type, 2, 2));
            game.state.board.plants[0].launch_counter = launch_counter;
        ...
        for (target_x, expected_motion) in [
            ...
            let launch_counter = if expected_motion == ProjectileMotion::Straight {
                26
            } else {
                1
            };
            let (mut game, events) = run(PlantType::Other(28), launch_counter, &[(2, target_x)]);
            assert_eq!(
                events
                    .iter()
        ...
        let (no_target, events) = run(PlantType::Other(28), 1, &[(1, 500 * POSITION_SCALE)]);
        assert!(
            events
                .iter()
        ...
        let (backward, events) = run(PlantType::Other(52), 1, &[(2, 100 * POSITION_SCALE)]);
        assert_eq!(
            events
                .iter()
                .filter(|event| matches!(event, GameEvent::ProjectileFired { .. }))
        ...
        let (forward, events) = run(PlantType::Other(52), 1, &[(2, 500 * POSITION_SCALE)]);
        assert!(
            events
                .iter()
                .all(|event| !matches!(event, GameEvent::PlantFired { .. }))
    ...
    #[test]
    fn plant_specific_firing_cadence_matches_source_counters() {
        let run = |plant_type: PlantType, target_x, ticks| {
            let mut game = Game::new(7, SceneKind::Day);
            game.state
                .board
                .plants
            ...
            game.state.board.plants[0].launch_counter = 1;
            let mut setup = Vec::new();
            game.spawn_normal_zombie(2, 0, Some(target_x), &mut setup);
            ...
            for tick in 1..=ticks {
                let events = game.advance(InputFrame::default());
                if tick == 1 {
                    reset_counter = game.state.board.plants[0].launch_counter;
                }
                for event in events {
                    if matches!(event, GameEvent::ProjectileFired { source: 100, .. }) {
                        fired.push(tick);
                    }
                }
            }
        ...
        for (plant_type, target_x, ticks, expected) in [
            (PlantType::Peashooter, 500 * POSITION_SCALE, 60, vec![33]),
            (
                PlantType::Other(40),
                500 * POSITION_SCALE,
                100,
            ...
            (PlantType::Other(32), 500 * POSITION_SCALE, 60, vec![32]),
            (
                PlantType::Other(42),
                250 * POSITION_SCALE,
                180,
            ...
            (PlantType::Other(10), 500 * POSITION_SCALE, 60, vec![50]),
        ] {
            assert_eq!(run(plant_type, target_x, ticks).0, expected);
        }

        let (repeater, repeater_cycle) = run(PlantType::Other(7), 500 * POSITION_SCALE, 200);
        assert_eq!(repeater, vec![26, repeater_cycle + 1, repeater_cycle + 26]);

        let (cattail, cattail_cycle) = run(PlantType::Other(43), 500 * POSITION_SCALE, 200);
        assert_eq!(cattail, vec![32, cattail_cycle - 18, cattail_cycle + 32]);

        let mut cattail = Game::new(7, SceneKind::Day);
        cattail.state.board.plants = vec![test_plant(100, PlantType::Other(43), 2, 2)];
        cattail.state.board.plants[0].launch_counter = 1;
        let mut setup = Vec::new();
        cattail.spawn_normal_zombie(4, 0, Some(500 * POSITION_SCALE), &mut setup);
        cattail.advance(InputFrame::default());
        cattail.state.board.zombies.clear();
        assert!(
            (0..31)
    ...
    #[test]
    fn repeater_uses_the_launch_counter_second_shot() {
        let mut game = Game::new(7, SceneKind::Day);
        game.state.sun = 250;
        game.advance(InputFrame {
            actions: vec![
    ...
    fn snorkel_uses_source_spawn_speed_and_hides_in_pool_until_it_eats() {
        ...
        let entry_start = game.advance(InputFrame::default());
        assert_eq!(
            game.state
                .board
                .zombies
        ...
        assert!(!entry_start.iter().any(|event| matches!(
            event,
            GameEvent::ZombieEnteredPool { entity, .. } if *entity == snorkel
        )));
        let splash_events = (0..SNORKEL_SPLASH_TICKS)
            .flat_map(|_| game.advance(InputFrame::default()))
            .collect::<Vec<_>>();
        assert!(splash_events.iter().any(|event| matches!(
            event,
            GameEvent::ZombieEnteredPool { entity, .. } if *entity == snorkel
        )));
        let state = game
            ...
            .find(|candidate| candidate.id == snorkel)
            .unwrap();
        assert_eq!(state.snorkel_phase, SNORKEL_INTO_POOL_PHASE);
        assert!(!state.in_pool);

        let entry_events = (SNORKEL_SPLASH_TICKS..SNORKEL_ENTRY_TICKS)
            .flat_map(|_| game.advance(InputFrame::default()))
            .collect::<Vec<_>>();
        assert!(!entry_events.iter().any(|event| matches!(
            event,
        ...
        for _ in 0..SNORKEL_EAT_TRANSITION_TICKS {
            game.advance(InputFrame::default());
        }
        assert_eq!(
            game.state
        ...
        let snorkel_index = game
            ...
            .zombies
            .iter()
            .position(|candidate| candidate.id == snorkel)
            .unwrap();
        {
            let snorkel_state = &mut game.state.board.zombies[snorkel_index];
            snorkel_state.position_x = 25 * POSITION_SCALE;
            snorkel_state.speed = 0;
        }
        let mut exit_events = Vec::new();
        game.update_snorkel_state(snorkel_index, &mut exit_events);
        assert_eq!(
            game.state.board.zombies[snorkel_index].snorkel_phase,
            SNORKEL_OUT_OF_POOL_PHASE
        );
        assert!(game.state.board.zombies[snorkel_index].in_pool);
        assert_eq!(
            game.state.board.zombies[snorkel_index].position_x,
            10 * POSITION_SCALE
        );
        ...
        let mut setup = Vec::new();
        let backwards_id =
            backwards.spawn_snorkel_zombie(2, 0, Some(641 * POSITION_SCALE), &mut setup);
        let backwards_index = backwards
            .state
            .board
            .zombies
        ...
        let mut backwards_events = Vec::new();
        backwards.update_snorkel_state(backwards_index, &mut backwards_events);
        assert_eq!(
            backwards.state.board.zombies[backwards_index].snorkel_phase,
            SNORKEL_OUT_OF_POOL_PHASE
        );
        assert_eq!(
            backwards.state.board.zombies[backwards_index].position_x,
            656 * POSITION_SCALE
        );
        for _ in 0..SNORKEL_EXIT_TICKS {
            backwards.update_snorkel_state(backwards_index, &mut backwards_events);
    ...
    #[test]
    fn backup_dancer_rise_rejects_ground_damage_until_landing() {
        let backup_x = grid_x(4);
        let mut game = Game::new(7, SceneKind::Day);
        let mut setup = Vec::new();
        let backup = game.spawn_backup_dancer(1, 0, Some(backup_x), &mut setup);
        game.state.board.zombies[0].speed = 0;
        let starting_health = game.state.board.zombies[0].health;
        let rising = &game.state.board.zombies[0];
        assert_eq!(rising.dancer_phase, DANCER_RISE_PHASE);
        assert!(!projectile_can_hit_zombie(rising, ProjectileType::Pea));
        assert!(!projectile_can_hit_zombie(rising, ProjectileType::Fireball));
        assert!(!squash_hits_zombie(rising, 1, backup_x));
        ...
        assert!(!events.iter().any(|event| matches!(
            event,
            GameEvent::ProjectileHit { zombie, .. } if *zombie == backup
        )));
        assert_eq!(game.state.board.zombies[0].health, starting_health);
        game.state.board.projectiles.clear();

        let normal = game.spawn_normal_zombie(2, 0, Some(backup_x), &mut setup);
        game.state.board.zombies[1].speed = 0;
        events.clear();
        game.fire_projectile(
            0,
        ...
        assert!(!events.iter().any(|event| matches!(
            event,
            GameEvent::ProjectileSplashHit { zombie, .. } if *zombie == backup
        )));
        assert_eq!(game.state.board.zombies[0].health, starting_health);
        ...
        let landed = game
            ...
            .zombies
            .iter()
            .find(|zombie| zombie.id == backup)
            .unwrap();
        assert_eq!(landed.dancer_phase, DANCER_DANCE_PHASE);
        assert!(projectile_can_hit_zombie(landed, ProjectileType::Pea));
        assert!(squash_hits_zombie(landed, 1, landed.position_x));
        let landed_x = landed.position_x;
        ...
        assert!(events.iter().any(|event| matches!(
            event,
            GameEvent::ProjectileHit { zombie, damage: 20, .. } if *zombie == backup
        )));
        assert_eq!(
            game.state
                .board
        ...
        let cob_target = cob_game.spawn_backup_dancer(1, 0, Some(grid_x(4)), &mut cob_events);
